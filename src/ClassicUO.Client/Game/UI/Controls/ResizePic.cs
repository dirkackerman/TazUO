...
public class ResizePic : Control
{
    ...
    public override bool Contains(int x, int y)
    {
        ...
        if (PixelsInXY(ref bounds0, Graphic, x, y)) return true;

        int DW = Width - bounds0.Width - bounds2.Width;

        if (DW >= 1 && PixelsInXY(ref bounds1, (ushort)(Graphic + 1), x - bounds0.Width, y, DW)) return true;
        ...
        if (PixelsInXY(ref bounds5, (ushort)(Graphic + 6), x, y - (Height - bounds5.Height))) return true;
    ...
    {
        if (x < 0 || y < 0 || width > 0 && x >= width || height > 0 && y >= height) return false;
    ...
    private void DrawInternal(UltimaBatcher2D batcher, int x, int y, Vector3 color, float layerDepth)
    {
        ...
        if (texture0 != null) batcher.Draw(texture0, new Vector2(x, y), bounds0, color, layerDepth);
