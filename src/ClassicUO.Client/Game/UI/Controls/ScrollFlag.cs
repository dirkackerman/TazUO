...
public class ScrollFlag : ScrollBarBase
{
    ...
    public override bool AddToRenderLists(RenderLists renderLists, int x, int y, ref float layerDepthRef)
    {
        ...
        SpriteInfo gumpInfoFlag = Client.Game.UO.Gumps.GetGump(BUTTON_FLAG);
        SpriteInfo gumpInfoUp = Client.Game.UO.Gumps.GetGump(BUTTON_UP);
        SpriteInfo gumpInfoDown = Client.Game.UO.Gumps.GetGump(BUTTON_DOWN);
        ...
        (
            (batcher) =>
            {
                ...
                if (_showButtons)
                {
                    if (gumpInfoUp.Texture != null) batcher.Draw(gumpInfoUp.Texture, new Vector2(x, y), gumpInfoUp.UV, hueVector, layerDepth);
    ...
    protected override void CalculateByPosition(int x, int y)
    {
        if (y != _clickPosition.Y)
        {
            ...
            else if (y == scrollableArea && _clickPosition.Y > Height - (height >> 1)) _clickPosition.Y = Height - (height >> 1);
    ...
    public override bool Contains(int x, int y)
    {
        ...
        if (gumpInfoFlag.Texture == null) return false;
