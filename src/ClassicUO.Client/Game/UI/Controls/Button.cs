...
public class Button : Control
{
    ...
    )
    {
        ...
        if (!string.IsNullOrEmpty(caption) && normalHue != ushort.MaxValue)
        {
            ...
            if (hoverHue != ushort.MaxValue) _fontTexture[1] = RenderedText.Create(caption, HueHover, font, isunicode);
        }
    ...
    public override bool AddToRenderLists(RenderLists renderLists, int x, int y, ref float layerDepthRef)
    {
        ...
        renderLists.AddGumpWithAtlas(
            batcher =>
            {
                batcher.Draw(texture, new Rectangle(x, y, Width, Height), bounds, hueVector, layerDepth);
                return true;
            });
        ...
        if (!string.IsNullOrEmpty(_caption))
        {
            ...
            if (FontCenter)
            {
                ...
                renderLists.AddGumpNoAtlas(
                    batcher => textTexture.Draw(batcher, x, y, layerDepth)
                );
        }
